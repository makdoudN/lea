...
def train(conf):
    ...
    library = load_library(conf)
    memory = ReplayBuffer(**conf.memory.kwargs)

    kwargs = {}
    if ("pr" in conf.rl.cls) or ("lea" in conf.rl.cls):
        kwargs["library"] = library
    elif "rpl" in conf.rl.cls:
        assert len(library) == 1
        kwargs["base_policy"] = library[0]
    else:
        raise ValueError()
    ...
    for e in range(conf.num_epochs):
        ...
        if "count_advice" in model.__dict__:
            count_advice = model.count_advice.copy()
            model.count_advice = model.count_advice * 0          # Reset Advice
            epoch_info["count_advice"] = count_advice
